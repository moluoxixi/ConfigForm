...
- `required` 只表达必填标识和空值校验；字符串会先 trim 再判断空值，更复杂的格式、长度或跨字段规则继续放在 `schema` / `validator`。
...
- runtime 对组件和插件名注册冲突一律抛错，不提供覆盖或静默降级策略；插件通过 `getDefaultField(field)` 提供默认字段片段，通过 `transformField(field)` 处理合并后的字段。
