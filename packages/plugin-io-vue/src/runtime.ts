import { vueFormIOPlugin } from './plugin'
...
import type { VueFormIOPluginOptions, VueFormIORuntime } from './types'
...
function requireApi(api: FormIOPluginAPI | undefined): FormIOPluginAPI {
  if (!api) {
    throw new Error('[plugin-io-vue] Runtime API is not ready. Mount ConfigForm with runtime.plugin first.')
  }
  return api
}
...
export function createVueFormIORuntime(options: VueFormIOPluginOptions = {}): VueFormIORuntime {
  const rawPlugin = vueFormIOPlugin(options)
