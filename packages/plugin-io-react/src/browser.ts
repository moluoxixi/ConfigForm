...
function assertBrowserAPI(name: string, target: unknown): void {
  if (!target) {
    throw new Error(`[plugin-io-react] ${name} is not available in current environment.`)
  }
}
...
export async function browserPrint(payload: FormIOPrintPayload): Promise<void> {
  ...
  if (!printWindow) {
    throw new Error('[plugin-io-react] Failed to open print window. Please allow popups and try again.')
  }
...
export async function readFileAsText(file: File): Promise<string> {
  ...
  return await new Promise<string>((resolve, reject) => {
    ...
    reader.onerror = () => {
      reject(reader.error ?? new Error('[plugin-io-react] Failed to read file.'))
    }
