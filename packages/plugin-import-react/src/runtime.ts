...
import type { ReactFormImportPluginOptions, ReactFormImportRuntime } from './types'
import { readFileAsText } from './browser'
import { reactFormImportPlugin } from './plugin'

function requireApi(api: FormImportPluginAPI | undefined): FormImportPluginAPI {
  if (!api) {
    throw new Error('[plugin-import-react] Runtime API is not ready. Mount ConfigForm with runtime.plugin first.')
  }
  return api
}

export function createReactFormImportRuntime(options: ReactFormImportPluginOptions = {}): ReactFormImportRuntime {
  const rawPlugin = reactFormImportPlugin(options)
