  ...
  setup(props) {
    ...
    const handleParseFile = async (file: File): Promise<void> => {
      try {
        ...
        ElMessage.info(`JSON 解析完成：可导入 ${parsed.appliedKeys.length} 个字段`)
      }
      catch (error) {
        const text = error instanceof Error ? error.message : String(error)
        ElMessage.error(`导入失败：${text}`)
      }
    }
    ...
    const readEditorData = (): Record<string, unknown> | null => {
      ...
      try {
        ...
        ElMessage.error(`JSON 解析失败：${text}`)
        return null
      }
    }
    ...
    const confirmImport = async (): Promise<void> => {
      try {
        ...
        ElMessage.success(`JSON 导入成功：已更新 ${applied.appliedKeys.length} 个字段`)
      }
      catch (error) {
        const text = error instanceof Error ? error.message : String(error)
        errorMessage.value = text
    ...
    const renderSourceModal = (): ReturnType<typeof h> => {
      return h(ElDialog, {
        ...
        footer: () => [
          h(ElButton, { onClick: () => { sourceOpen.value = false } }, () => props.cancelText),
        ],
      })
    }
