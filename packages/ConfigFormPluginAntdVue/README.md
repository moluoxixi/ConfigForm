...
ConfigForm 的 Ant Design Vue 运行时适配插件。核心包的字段组件默认使用 `modelValue/update:modelValue` 作为绑定协议；本插件通过 `getDefaultField(field)` 生命周期把 Ant Design Vue 字段组件映射到对应的 `valueProp/trigger`。
