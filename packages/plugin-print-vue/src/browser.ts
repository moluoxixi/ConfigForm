...
function assertBrowserAPI(name: string, target: unknown): void {
  if (!target) {
    throw new Error(`[plugin-print-vue] ${name} is not available in current environment.`)
  }
}
...
export async function browserPrint(payload: FormPrintPayload): Promise<void> {
  ...
  if (!printWindow) {
    throw new Error('[plugin-print-vue] Failed to open print window. Please allow popups and try again.')
  }
