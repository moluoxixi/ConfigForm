...
import type { VueFormExportPluginOptions, VueFormExportRuntime } from './types'
import { vueFormExportPlugin } from './plugin'
...
function requireApi(api: FormExportPluginAPI | undefined): FormExportPluginAPI {
  if (!api) {
    throw new Error('[plugin-export-vue] Runtime API is not ready. Mount ConfigForm with runtime.plugin first.')
  }
  return api
}
...
export function createVueFormExportRuntime(options: VueFormExportPluginOptions = {}): VueFormExportRuntime {
  const rawPlugin = vueFormExportPlugin(options)
