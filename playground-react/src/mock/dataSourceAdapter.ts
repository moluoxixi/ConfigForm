 ...
 * Mock 请求适配器（React 端）
 *
 * 与 Vue 端共用相同的 mock 数据和 API 路由。
 * 通过 registerRequestAdapter('mock', ...) 注册到核心库。
 */
...
const routes: Record<string, (params: Record<string, unknown>) => unknown> = {
  '/api/models': params => MODELS[params.brand as string] ?? [],
  '/api/configs': params => CONFIGS[params.model as string] ?? [],
  '/api/classes': params => CLASSES[params.grade as string] ?? [],
  '/api/users': (params) => {
...
export function setupMockAdapter(): void {
  ...
  registerRequestAdapter('mock', {
    async request<T>(config: RequestConfig): Promise<T> {
      ...
      logs.push(`${logEntry} → 200 OK (${JSON.stringify(data).length} bytes)`)
