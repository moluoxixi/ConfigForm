            ...
            <FormProvider form={form}>
              <form onSubmit={async (e: React.FormEvent) => {
                ...
                <FormField name="licensePlate" fieldProps={{
                  label: '车牌号',
                  component: 'Input',
                  componentProps: { placeholder: '如：京A12345' },
                  rules: [{ pattern: /^[\u4E00-\u9FA5][A-Z][A-Z0-9]{5}$/, message: '请输入有效车牌号（如：京A12345）' }],
                }}
                />
                ...
                <FormField name="phone" fieldProps={{
                  ...
                  required: true,
                  component: 'Input',
                  componentProps: { placeholder: '请输入中国大陆手机号' },
                  rules: [{
                    validator: (value) => {
                      if (!value) return undefined
                      if (!CHINA_PHONE_REGEX.test(String(value))) return '请输入有效的中国大陆手机号（1开头，11位数字）'
                      return undefined
                    },
                  }],
                }}
                ...
                <FormField name="password" fieldProps={{
                  ...
                  required: true,
                  component: 'Password',
                  componentProps: { placeholder: '8-32 位，含大小写和数字' },
                  rules: [
                    { stopOnFirstFailure: true, minLength: 8, maxLength: 32, message: '密码长度 8-32 个字符' },
                    { pattern: /[a-z]/, message: '需包含至少一个小写字母' },
                    { pattern: /[A-Z]/, message: '需包含至少一个大写字母' },
                    { pattern: /\d/, message: '需包含至少一个数字' },
                    {
                      validator: (value) => {
                        if (WEAK_PASSWORDS.includes(String(value).toLowerCase())) return '密码过于简单，请使用更复杂的密码'
                        return undefined
                      },
                    },
                  ],
                  ...
                  rules: [
                    { min: 0, max: 150, message: '年龄范围 0-150' },
                    {
                      ...
                      validator: (value) => {
                        ...
                        if (age > 0 && age < 18) return '未成年用户部分功能可能受限'
                        if (age > 60) return '建议开启大字模式以获得更好体验'
                        return undefined
                      },
                    },
                  ],
                ...
                {/* 条件验证：证件类型切换 */}
                <FormField name="idType" fieldProps={{
                  label: '证件类型',
                  required: true,
                  component: 'Select',
                  dataSource: [
                ...
                />
                <FormField name="idNumber" fieldProps={{
                  label: '证件号码',
                  required: true,
                  component: 'Input',
                  componentProps: { placeholder: '请输入身份证号码' },
                  reactions: [{
                    watch: 'idType',
                    fulfill: {
                      run: (field, ctx) => {
                ...
                {/* 自定义格式验证：IP 地址 */}
                <FormField name="ipAddress" fieldProps={{
                  label: 'IP 地址',
                  component: 'Input',
                  componentProps: { placeholder: '如：192.168.1.1' },
                  rules: [{
                    validator: (value) => {
                      if (!value) return undefined
