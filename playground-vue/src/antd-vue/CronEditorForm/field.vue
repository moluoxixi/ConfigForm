    ...
    <StatusTabs ref="st" v-slot="{ mode, showResult }">
      <FormProvider :form="form">
        ...
        <FormField v-slot="{ field }" name="cronExpr">
          <AFormItem :label="field.label">
            <span v-if="mode === 'readOnly'">{{ (field.value as string) || '—' }}</span><AInput v-else :value="(field.value as string) ?? ''" :disabled="mode === 'disabled'" placeholder="如：0 8 * * 1-5" addon-before="Cron" style="width: 400px" @update:value="field.setValue($event)" />
            <div style="margin-top: 8px">
              <span style="color: #999">解析：</span><ATag :color="describeCron((field.value as string) ?? '').includes('错误') ? 'error' : 'blue'">
                {{ describeCron((field.value as string) ?? '') }}
              </ATag>
            </div>
            <div v-if="mode === 'editable'" style="margin-top: 8px">
              <span style="color: #999; font-size: 12px">快捷预设：</span><ASpace wrap>
                <ATag v-for="p in PRESETS" :key="p.value" :color="field.value === p.value ? 'blue' : 'default'" style="cursor: pointer" @click="field.setValue(p.value)">
                  {{ p.label }}
                </ATag>
              </ASpace>
            </div>
