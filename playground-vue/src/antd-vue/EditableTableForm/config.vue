            ...
            <table style="width: 100%; border-collapse: collapse; border: 1px solid #f0f0f0">
              ...
              <tbody>
                <tr v-for="(_, idx) in ((arrayField.value as unknown[]) ?? [])" :key="idx" style="border-bottom: 1px solid #f0f0f0">
                  <td style="padding: 6px 8px; text-align: center; color: #999">
                    {{ idx + 1 }}
                  </td>
                  <td style="padding: 6px 8px">
                    <FormField v-slot="{ field }" :name="`items.${idx}.productName`">
                      <span v-if="mode === 'readOnly'">{{ (field.value as string) || '—' }}</span><AInput v-else :value="(field.value as string) ?? ''" placeholder="商品" size="small" :disabled="mode === 'disabled'" @update:value="field.setValue($event)" />
                    </FormField>
                  </td>
                  <td style="padding: 6px 8px">
                    <FormField v-slot="{ field }" :name="`items.${idx}.quantity`">
                      <span v-if="mode === 'readOnly'">{{ field.value ?? '—' }}</span><AInputNumber v-else :value="(field.value as number) ?? 1" :min="1" size="small" style="width: 100%" :disabled="mode === 'disabled'" @update:value="(v: number | null) => { field.setValue(v ?? 1); form.setFieldValue(`items.${idx}.subtotal`, (v ?? 1) * ((form.getFieldValue(`items.${idx}.unitPrice`) as number) ?? 0)) }" />
                    </FormField>
                  </td>
                  <td style="padding: 6px 8px">
                    <FormField v-slot="{ field }" :name="`items.${idx}.unitPrice`">
                      <span v-if="mode === 'readOnly'">{{ field.value ?? '—' }}</span><AInputNumber v-else :value="(field.value as number) ?? 0" :min="0" :step="0.01" size="small" style="width: 100%" :disabled="mode === 'disabled'" @update:value="(v: number | null) => { field.setValue(v ?? 0); form.setFieldValue(`items.${idx}.subtotal`, ((form.getFieldValue(`items.${idx}.quantity`) as number) ?? 0) * (v ?? 0)) }" />
                    </FormField>
                  </td>
                  <td style="padding: 6px 8px; text-align: right">
                    <FormField v-slot="{ field }" :name="`items.${idx}.subtotal`">
                      <strong style="color: #52c41a">¥{{ ((field.value as number) ?? 0).toFixed(2) }}</strong>
                    </FormField>
                  </td>
