...
const schema: ISchema = {
  ...
  properties: {
    ...
    city: { type: 'string', title: '城市', required: true, placeholder: '请先选择省份', reactions: [{ watch: 'province', fulfill: { run: (f: any, ctx: any) => { const p = ctx.values.province as string; f.setValue(undefined); f.setDataSource(p ? (CITIES[p] ?? []) : []); f.setComponentProps({ placeholder: p ? '请选择城市' : '请先选择省份' }) } } }] },
    district: { type: 'string', title: '区县', placeholder: '请先选择城市', reactions: [{ watch: 'city', fulfill: { run: (f: any, ctx: any) => { const c = ctx.values.city as string; f.setValue(undefined); f.setDataSource(c ? (DISTRICTS[c] ?? []) : []); f.setComponentProps({ placeholder: c ? '请选择区县' : '请先选择城市' }) } } }] },
    categoryL1: { type: 'string', title: '一级分类', required: true, placeholder: '请选择', enum: CAT_L1 },
    categoryL2: { type: 'string', title: '二级分类', required: true, placeholder: '请先选择一级', reactions: [{ watch: 'categoryL1', fulfill: { run: (f: any, ctx: any) => { const l1 = ctx.values.categoryL1 as string; f.setValue(undefined); f.setDataSource(l1 ? (CAT_L2[l1] ?? []) : []) } } }] },
    categoryL3: { type: 'string', title: '三级分类', placeholder: '请先选择二级', reactions: [{ watch: 'categoryL2', fulfill: { run: (f: any, ctx: any) => { const l2 = ctx.values.categoryL2 as string; f.setValue(undefined); f.setDataSource(l2 ? (CAT_L3[l2] ?? []) : []) } } }] },
  },
}
</script>
