...
const scenarioSchemas: Record<string, Partial<FormSchema>> = {
  ...
  customer: {
    fields: {
      ...
      source: {
        type: 'string', label: '来源渠道', component: 'RadioGroup', wrapper: 'FormItem',
  ...
  supplier: {
    fields: {
      ...
      licenseNumber: {
        type: 'string', label: '许可证号', component: 'Input', wrapper: 'FormItem', required: true, placeholder: '请输入营业执照号',
...
<template>
  <div>
    <h2 style="margin-bottom: 8px;">Ant Design Vue 纯配置 - 动态 Schema</h2>
    ...
    <div style="margin-bottom: 20px; display: flex; align-items: center; gap: 12px;">
      ...
      <a-radio-group :value="currentScenario" @update:value="switchScenario($event as ScenarioKey)" button-style="solid">
        <a-radio-button v-for="s in scenarioButtons" :key="s.key" :value="s.key">{{ s.label }}</a-radio-button>
      </a-radio-group>
    </div>
    ...
    <details style="margin-top: 20px;">
      <summary style="cursor: pointer; color: #1677ff; font-size: 14px;">查看当前合并后的 Schema</summary>
