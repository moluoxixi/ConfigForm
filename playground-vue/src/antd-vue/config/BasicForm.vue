 ...
 *
 * 演示：通过 ConfigForm + Schema 配置驱动，使用 Ant Design Vue 全套组件
 * - 各类型字段：Input / Password / Textarea / InputNumber / Select / RadioGroup / Switch / DatePicker
 * - required 必填验证
 * - 格式验证（email）
 * - 字符串长度 / 数值范围
 */
...
const schema: FormSchema = {
  fields: {
    username: {
      ...
      required: true,
      placeholder: '请输入用户名',
      rules: [{ minLength: 2, maxLength: 20, message: '用户名 2-20 个字符' }],
    },
    password: {
      type: 'string',
      label: '密码',
