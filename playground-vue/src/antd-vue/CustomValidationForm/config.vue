...
const schema: ISchema = {
  ...
  properties: {
    ...
    phone: { type: 'string', title: '手机号', required: true, placeholder: '中国大陆手机号', rules: [{ validator: (v: unknown) => { if (!v) return undefined; if (!/^1[3-9]\d{9}$/.test(String(v))) return '无效大陆手机号'; return undefined } }] },
    ...
    age: { type: 'number', title: '年龄', required: true, rules: [{ min: 0, max: 150, message: '0-150' }, { level: 'warning', validator: (v: unknown) => { const a = Number(v); if (a > 0 && a < 18) return '未成年部分功能受限'; if (a > 60) return '建议开启大字模式'; return undefined } }] },
