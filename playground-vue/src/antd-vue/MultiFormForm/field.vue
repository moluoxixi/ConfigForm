  ...
  <div>
    ...
    <StatusTabs ref="st" v-slot="{ mode, showResult }">
      <FormProvider :form="mainForm">
              ...
              <FormField v-for="n in ['orderName', 'customer', 'total']" :key="n" v-slot="{ field }" :name="n">
                <AFormItem :label="field.label">
                  <template v-if="mode === 'readOnly'">
                    ...
                    <AInputNumber v-if="n === 'total'" :value="(field.value as number)" :disabled="mode === 'disabled'" :min="0" style="width: 100%" @update:value="field.setValue($event)" /><AInput v-else :value="(field.value as string) ?? ''" :disabled="mode === 'disabled'" @update:value="field.setValue($event)" />
                  </template>
                </AFormItem>
              </FormField><AButton v-if="mode === 'editable'" type="dashed" @click="modalOpen = true">
                从弹窗填写联系人
              </AButton>
            </ACard>
          </ACol>
              ...
              <FormProvider :form="subForm">
                <FormField v-for="n in ['contactName', 'contactPhone', 'contactEmail']" :key="n" v-slot="{ field }" :name="n">
                  <AFormItem :label="field.label" :required="field.required" :validate-status="field.errors.length > 0 ? 'error' : undefined" :help="field.errors[0]?.message">
                    <span v-if="mode === 'readOnly'">{{ (field.value as string) || '—' }}</span><AInput v-else :value="(field.value as string) ?? ''" :disabled="mode === 'disabled'" @update:value="field.setValue($event)" @blur="field.blur(); field.validate('blur').catch(() => {})" />
                  </AFormItem>
                </FormField>
              </FormProvider>
            </ACard>
        ...
        <div v-if="mode === 'editable'" style="margin-top: 16px; display: flex; gap: 8px">
          ...
          <button type="button" style="padding: 4px 15px; background: #fff; border: 1px solid #d9d9d9; border-radius: 6px; cursor: pointer" @click="mainForm.reset(); subForm.reset()">
            重置
          </button>
        </div>
      </FormProvider>
    ...
    <AModal v-model:open="modalOpen" title="编辑联系人" @ok="modalOk">
      <FormProvider :form="subForm">
        <FormField v-for="n in ['contactName', 'contactPhone', 'contactEmail']" :key="n" v-slot="{ field }" :name="n">
          <AFormItem :label="field.label">
            <AInput :value="(field.value as string) ?? ''" @update:value="field.setValue($event)" />
          </AFormItem>
        </FormField>
      </FormProvider>
    </AModal>
...
import { Button as AButton, Card as ACard, Col as ACol, FormItem as AFormItem, Input as AInput, InputNumber as AInputNumber, Modal as AModal, Row as ARow } from 'ant-design-vue'
