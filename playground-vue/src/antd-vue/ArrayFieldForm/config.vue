      ...
      <FormProvider :form="form">
        <FormField v-slot="{ field }" name="groupName">
          <AFormItem :label="field.label" :required="field.required">
            <AInput :value="(field.value as string) ?? ''" :disabled="mode === 'disabled'" :readonly="mode === 'readOnly'" style="width: 300px" @update:value="field.setValue($event)" />
          </AFormItem>
        </FormField>
          ...
          <div style="margin-bottom: 16px">
            ...
            <div v-for="(_, idx) in ((arrayField.value as unknown[]) ?? [])" :key="idx" :style="{ display: 'grid', gridTemplateColumns: '40px 1fr 1fr 1fr auto', gap: '8px', padding: '8px 12px', background: idx % 2 === 0 ? '#fafafa' : '#fff', borderRadius: '4px', alignItems: 'center' }">
              ...
              <FormField v-slot="{ field }" :name="`contacts.${idx}.name`">
                <span v-if="mode === 'readOnly'">{{ (field.value as string) || '—' }}</span><AInput v-else :value="(field.value as string) ?? ''" placeholder="姓名" size="small" :disabled="mode === 'disabled'" @update:value="field.setValue($event)" />
              </FormField>
              <FormField v-slot="{ field }" :name="`contacts.${idx}.phone`">
                <span v-if="mode === 'readOnly'">{{ (field.value as string) || '—' }}</span><AInput v-else :value="(field.value as string) ?? ''" placeholder="电话" size="small" :disabled="mode === 'disabled'" @update:value="field.setValue($event)" />
              </FormField>
              <FormField v-slot="{ field }" :name="`contacts.${idx}.email`">
                <span v-if="mode === 'readOnly'">{{ (field.value as string) || '—' }}</span><AInput v-else :value="(field.value as string) ?? ''" placeholder="邮箱" size="small" :disabled="mode === 'disabled'" @update:value="field.setValue($event)" />
              </FormField>
              <ASpace v-if="mode === 'editable'" :size="4">
                <AButton size="small" :disabled="idx === 0" @click="arrayField.moveUp(idx)">
                  ↑
                </AButton>
                <AButton size="small" :disabled="idx === ((arrayField.value as unknown[]) ?? []).length - 1" @click="arrayField.moveDown(idx)">
                  ↓
                </AButton>
...
import { Button as AButton, FormItem as AFormItem, Input as AInput, Space as ASpace } from 'ant-design-vue'
