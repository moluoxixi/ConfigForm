<template>
  <div>
    ...
    <ARow :gutter="16">
      <ACol :span="12"><ACard title="主表单 - 订单信息" size="small"><FormProvider :form="mainForm"><FormField v-for="n in ['orderName','customer','total']" :key="n" v-slot="{ field }" :name="n"><AFormItem :label="field.label"><AInputNumber v-if="n === 'total'" :value="(field.value as number)" @update:value="field.setValue($event)" :disabled="mode === 'disabled'" :min="0" style="width: 100%" /><AInput v-else :value="(field.value as string) ?? ''" @update:value="field.setValue($event)" :disabled="mode === 'disabled'" /></AFormItem></FormField><AButton v-if="mode === 'editable'" type="dashed" @click="modalOpen = true">从弹窗填写联系人</AButton></FormProvider></ACard></ACol>
      <ACol :span="12"><ACard title="子表单 - 联系人" size="small"><FormProvider :form="subForm"><FormField v-for="n in ['contactName','contactPhone','contactEmail']" :key="n" v-slot="{ field }" :name="n"><AFormItem :label="field.label" :required="field.required" :validate-status="field.errors.length > 0 ? 'error' : undefined" :help="field.errors[0]?.message"><AInput :value="(field.value as string) ?? ''" @update:value="field.setValue($event)" @blur="field.blur(); field.validate('blur').catch(() => {})" :disabled="mode === 'disabled'" /></AFormItem></FormField></FormProvider></ACard></ACol>
    </ARow>
