...
const SCENARIOS: Record<ScenarioKey, { label: string; override: Partial<ISchema> }> = {
  individual: { label: '个人用户', override: { properties: { idCard: { type: 'string', title: '身份证', required: true, rules: [{ pattern: /^\d{17}[\dXx]$/, message: '无效身份证' }], order: 4 }, city: { type: 'string', title: '城市', enum: [{ label: '北京', value: 'bj' }, { label: '上海', value: 'sh' }], order: 5 } } } },
