        ...
        <StatusTabs ref="st" v-slot="{ mode, showResult }">
          <FormProvider :form="form">
            <ASpace style="margin-bottom: 12px">
              <span>自动保存：</span><ASwitch v-model:checked="autoSave" />
            </ASpace>
            <FormField v-for="n in FIELDS" :key="n" v-slot="{ field }" :name="n">
              <AFormItem :label="field.label">
                <template v-if="mode === 'readOnly'">
                  <span v-if="n === 'description'" style="white-space:pre-wrap">{{ (field.value as string) || '—' }}</span><span v-else>{{ field.value ?? '—' }}</span>
                </template>
                <template v-else>
                  <AInputNumber v-if="n === 'price'" :value="(field.value as number)" :disabled="mode === 'disabled'" style="width: 100%" @update:value="field.setValue($event)" />
                  <ATextarea v-else-if="n === 'description'" :value="(field.value as string) ?? ''" :disabled="mode === 'disabled'" :rows="3" @update:value="field.setValue($event)" />
      ...
      <ACard :title="`事件日志 (${logs.length})`" size="small" style="width: 360px">
        ...
        <div style="max-height: 400px; overflow: auto; font-size: 12px">
          <div v-for="log in logs" :key="log.id" style="padding: 2px 0; border-bottom: 1px solid #f0f0f0">
            <ATag :color="typeColors[log.type]" style="font-size: 10px">
              {{ log.type }}
            </ATag><span style="color: #999">{{ log.time }}</span><div style="color: #555; margin-top: 2px">
              {{ log.message }}
            </div>
          </div>
        </div>
...
import { Button as AButton, Card as ACard, FormItem as AFormItem, Input as AInput, InputNumber as AInputNumber, Space as ASpace, Switch as ASwitch, Tag as ATag, Textarea as ATextarea } from 'ant-design-vue'
