  ...
  <div>
    ...
    <StatusTabs ref="st" v-slot="{ mode, showResult }">
      <FormProvider :form="form">
        <ASpace style="margin-bottom: 16px"><span style="font-weight: 600">当前角色：</span><ASegmented v-model:value="role" :options="[{label:'管理员',value:'admin'},{label:'经理',value:'manager'},{label:'员工',value:'staff'},{label:'访客',value:'guest'}]" /></ASpace>
...
import { Space as ASpace, Segmented as ASegmented, Input as AInput, InputNumber as AInputNumber, FormItem as AFormItem, Card as ACard, Textarea as ATextarea, Tag as ATag } from 'ant-design-vue'
...
watch([role, () => form.pattern], () => { FIELD_DEFS.forEach(d => { const f = form.getField(d.name); if (!f) return; const perm = PERM_MATRIX[d.name]?.[role.value] ?? 'hidden'; f.visible = perm !== 'hidden'; if (form.pattern === 'editable') { f.pattern = (perm === 'readOnly' ? 'readOnly' : 'editable') as FieldPattern } else { f.pattern = form.pattern } }) }, { immediate: true })
