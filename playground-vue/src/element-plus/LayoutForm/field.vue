  ...
  <div>
    ...
    <div style="margin-bottom: 16px">
      ...
      <div style="display:inline-flex">
        <button v-for="(opt, idx) in LAYOUT_OPTIONS" :key="opt.value" type="button" :style="{ padding: '5px 15px', fontSize: '14px', border: '1px solid #dcdfe6', background: layoutType === opt.value ? '#409eff' : '#fff', color: layoutType === opt.value ? '#fff' : '#606266', cursor: 'pointer', marginLeft: idx > 0 ? '-1px' : '0', borderRadius: idx === 0 ? '4px 0 0 4px' : idx === LAYOUT_OPTIONS.length - 1 ? '0 4px 4px 0' : '0' }" @click="layoutType = opt.value as LayoutType">
          {{ opt.label }}
        </button>
      </div>
    </div>
