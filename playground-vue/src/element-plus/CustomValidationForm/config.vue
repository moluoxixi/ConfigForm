...
const schema = computed<FormSchema>(() => ({
  ...
  fields: {
    ...
    phone: { type: 'string', label: '手机号', required: true, component: 'Input', wrapper: 'FormItem', placeholder: '中国大陆手机号', rules: [{ validator: (v: unknown) => {
      if (!v)
        return undefined
    ...
    age: { type: 'number', label: '年龄', required: true, component: 'InputNumber', wrapper: 'FormItem', rules: [{ min: 0, max: 150, message: '0-150' }, { level: 'warning', validator: (v: unknown) => {
      const a = Number(v)
      if (a > 0 && a < 18)
        return '未成年部分功能受限'
      if (a > 60)
    ...
    idNumber: { type: 'string', label: '证件号码', required: true, component: 'Input', wrapper: 'FormItem', reactions: [{ watch: 'idType', fulfill: { run: (f: any, ctx: any) => {
    ...
    ipAddress: { type: 'string', label: 'IP 地址', component: 'Input', wrapper: 'FormItem', placeholder: '192.168.1.1', rules: [{ validator: (v: unknown) => {
      if (!v)
        return undefined
