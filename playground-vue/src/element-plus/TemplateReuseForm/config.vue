<template>
  <div>
    <h2>模板复用</h2>
    <p style="color: #909399; margin-bottom: 16px; font-size: 14px;">Schema 片段复用 + 继承覆盖 = 不同业务表单</p>
    <el-space direction="vertical" :style="{ width: '100%', marginBottom: '16px' }">
      ...
      <el-radio-group v-model="template" size="small" @change="result = ''">
        <el-radio-button v-for="opt in templateOptions" :key="opt.value" :value="opt.value">{{ opt.label }}</el-radio-button>
      </el-radio-group>
    </el-space>
    <el-space style="margin-bottom: 12px"><el-tag type="info">复用片段：个人信息 + 地址 + 备注</el-tag></el-space>
