      ...
      <form novalidate @submit.prevent="handleSubmit">
        <FormField v-slot="{ field }" name="teamName">
          <el-form-item :label="field.label" :required="field.required">
            <el-input :model-value="(field.value as string) ?? ''" style="width: 300px" :disabled="mode === 'disabled'" @update:model-value="field.setValue($event)" />
          </el-form-item>
        </FormField>
        <FormArrayField v-slot="{ arrayField }" name="contacts" :field-props="{ minItems: 1, maxItems: 10, itemTemplate: () => ({ name: '', role: '', phones: [{ number: '', label: '手机' }] }) }">
          <div>
            <div style="display: flex; justify-content: space-between; margin-bottom: 12px">
              ...
              <el-button v-if="mode === 'editable'" type="primary" :disabled="!arrayField.canAdd" @click="arrayField.push({ name: '', role: '', phones: [{ number: '', label: '手机' }] })">
                添加联系人
              </el-button>
            </div>
            <el-card v-for="(_, idx) in ((arrayField.value as unknown[]) ?? [])" :key="idx" shadow="never" :header="`联系人 #${idx + 1}`" style="margin-bottom: 12px">
              <template v-if="mode === 'editable'" #header>
                <div style="display: flex; justify-content: space-between; align-items: center">
                  <span>联系人 #{{ idx + 1 }}</span><el-button size="small" type="danger" :disabled="!arrayField.canRemove" @click="arrayField.remove(idx)">
                    删除
                  </el-button>
                </div>
              </template>
              <el-space>
                <FormField v-slot="{ field }" :name="`contacts.${idx}.name`">
                  <el-input :model-value="(field.value as string) ?? ''" placeholder="姓名" :disabled="mode === 'disabled'" @update:model-value="field.setValue($event)">
                    <template #prepend>
                      姓名
                    </template>
                  </el-input>
                ...
                <FormField v-slot="{ field }" :name="`contacts.${idx}.role`">
                  <el-input :model-value="(field.value as string) ?? ''" placeholder="角色" :disabled="mode === 'disabled'" @update:model-value="field.setValue($event)">
                    <template #prepend>
                      角色
                    </template>
                  </el-input>
              ...
              <FormArrayField v-slot="{ arrayField: phoneArray }" :name="`contacts.${idx}.phones`" :field-props="{ minItems: 1, maxItems: 5, itemTemplate: () => ({ number: '', label: '手机' }) }">
                <div style="padding: 8px 12px; background: #f5f7fa; border-radius: 4px; margin-top: 8px">
                  <div style="display: flex; justify-content: space-between; margin-bottom: 8px">
                    ...
                    <el-button v-if="mode === 'editable'" size="small" :disabled="!phoneArray.canAdd" @click="phoneArray.push({ number: '', label: '手机' })">
                      添加电话
                    </el-button>
                  </div>
                  <el-space v-for="(__, pIdx) in ((phoneArray.value as unknown[]) ?? [])" :key="pIdx" :size="4" style="width: 100%; margin-bottom: 4px">
                    <FormField v-slot="{ field }" :name="`contacts.${idx}.phones.${pIdx}.label`">
                      <el-input :model-value="(field.value as string) ?? ''" placeholder="标签" size="small" style="width: 80px" :disabled="mode === 'disabled'" @update:model-value="field.setValue($event)" />
                    </FormField>
                    <FormField v-slot="{ field }" :name="`contacts.${idx}.phones.${pIdx}.number`">
                      <el-input :model-value="(field.value as string) ?? ''" placeholder="号码" size="small" style="width: 180px" :disabled="mode === 'disabled'" @update:model-value="field.setValue($event)" />
                    </FormField>
                    <el-button v-if="mode === 'editable'" size="small" type="danger" :disabled="!phoneArray.canRemove" @click="phoneArray.remove(pIdx)">
                      删
                    </el-button>
                  </el-space>
                </div>
