<template>
  <div>
    ...
    <ElSpace direction="vertical" :style="{ width: '100%', marginBottom: '16px' }">
      ...
      <ElSpace>
        <span style="font-weight: 600">当前角色：</span><ElRadioGroup v-model="role" size="small">
          <ElRadioButton v-for="r in [{ label: '管理员', value: 'admin' }, { label: '经理', value: 'manager' }, { label: '员工', value: 'staff' }, { label: '访客', value: 'guest' }]" :key="r.value" :value="r.value">
            {{ r.label }}
          </ElRadioButton>
        </ElRadioGroup>
      </ElSpace>
    ...
    <ElCard shadow="never" style="margin-bottom: 16px">
      <span style="font-weight: 600">权限矩阵（当前角色：{{ role }}）</span><div style="display: flex; gap: 8px; flex-wrap: wrap; margin-top: 8px">
        <ElTag v-for="d in FIELD_DEFS" :key="d.name" :type="PERM_MATRIX[d.name]?.[role] === 'editable' ? 'success' : PERM_MATRIX[d.name]?.[role] === 'readOnly' ? 'warning' : 'danger'">
    ...
    <FormProvider :form="form">
      <form novalidate @submit.prevent="handleSubmit">
        <FormField v-for="d in FIELD_DEFS" :key="d.name" v-slot="{ field }" :name="d.name">
          <ElFormItem v-if="field.visible" :label="field.label" :required="field.required">
            <ElInputNumber v-if="d.name === 'salary'" :model-value="(field.value as number)" :disabled="field.pattern === 'disabled' || mode === 'disabled'" :readonly="field.pattern === 'readOnly' || mode === 'readOnly'" style="width: 100%" @update:model-value="field.setValue($event)" />
            <ElInput v-else-if="d.name === 'remark'" type="textarea" :model-value="(field.value as string) ?? ''" :disabled="field.pattern === 'disabled' || mode === 'disabled'" :readonly="field.pattern === 'readOnly' || mode === 'readOnly'" @update:model-value="field.setValue($event)" />
            <ElInput v-else :model-value="(field.value as string) ?? ''" :disabled="field.pattern === 'disabled' || mode === 'disabled'" :readonly="field.pattern === 'readOnly' || mode === 'readOnly'" @update:model-value="field.setValue($event)" />
          </ElFormItem>
        </FormField>
...
import { ElAlert, ElButton, ElCard, ElFormItem, ElInput, ElInputNumber, ElRadioButton, ElRadioGroup, ElSpace, ElTag } from 'element-plus'
