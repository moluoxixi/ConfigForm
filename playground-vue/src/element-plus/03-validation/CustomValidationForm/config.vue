<template>
  <div>
    ...
    <p style="color: #909399; margin-bottom: 16px; font-size: 14px;">
      正则 / 自定义函数 / 多规则 / 警告级 / 条件规则 — ConfigForm + ISchema 实现
    </p>
...
/**
 * 自定义验证规则 — Config 模式（Element Plus）
 *
 * 使用 ConfigForm + ISchema 实现多种自定义验证：
 * - 正则验证（车牌号）
 * - 自定义函数验证（手机号）
 * - 多规则叠加（密码）
 * - 警告级验证（年龄）
...
const initialValues = {
  licensePlate: '', phone: '', password: '',
  age: undefined, idType: 'idcard', idNumber: '', ipAddress: '',
}
...
const schema: ISchema = {
  ...
  properties: {
    licensePlate: {
      type: 'string', title: '车牌号',
      componentProps: { placeholder: '京A12345' },
      rules: [{ pattern: /^[\u4E00-\u9FA5][A-Z][A-Z0-9]{5}$/, message: '无效车牌号' }],
    },
    phone: {
      type: 'string', title: '手机号', required: true,
      componentProps: { placeholder: '中国大陆手机号' },
      rules: [{
        validator: (v: unknown) => {
          if (!v) return undefined
          if (!/^1[3-9]\d{9}$/.test(String(v))) return '无效大陆手机号'
          return undefined
        },
      }],
    },
    password: {
      ...
      rules: [
        ...
        { validator: (v: unknown) => WEAK_PWD.includes(String(v).toLowerCase()) ? '密码过于简单' : undefined },
      ],
    },
    age: {
      type: 'number', title: '年龄', required: true, component: 'InputNumber',
      rules: [
    ...
    idType: {
      type: 'string', title: '证件类型', required: true, component: 'Select', default: 'idcard',
      enum: [{ label: '身份证', value: 'idcard' }, { label: '护照', value: 'passport' }, { label: '军官证', value: 'military' }],
    },
    idNumber: {
      type: 'string', title: '证件号码', required: true,
      componentProps: { placeholder: '18 位身份证' },
      reactions: [{
        watch: 'idType',
        fulfill: {
          run: (f: any, ctx: any) => {
    ...
    ipAddress: {
      type: 'string', title: 'IP 地址',
      componentProps: { placeholder: '192.168.1.1' },
      rules: [{
        validator: (v: unknown) => {
          if (!v) return undefined
          ...
          for (const p of parts) {
            const n = Number(p)
            if (Number.isNaN(n) || n < 0 || n > 255 || String(n) !== p) return '各段 0-255 整数'
          }
          return undefined
        },
      }],
