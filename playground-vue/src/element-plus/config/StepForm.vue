    ...
    <FormProvider :form="form">
      <form @submit.prevent="handleSubmit">
        ...
        <template v-for="(step, stepIdx) in steps" :key="stepIdx">
          <ElCard v-show="stepIdx === currentStep && stepIdx < 3" shadow="never">
            <template #header>
              ...
              <FormField v-for="fieldName in step.fields" :key="fieldName" v-slot="{ field }" :name="fieldName">
                ...
                >
                  ...
                  <ElDatePicker
                    ...
                    :model-value="(field.value as string)"
                    type="date"
                    placeholder="请选择日期" value-format="YYYY-MM-DD" style="width: 100%;" @update:model-value="field.setValue($event)"
                  />
                  <!-- 文本域 -->
                  <ElInput
        ...
        <!-- 预览确认页 -->
        <ElCard v-show="currentStep === 3" shadow="never">
          <template #header>
            <span style="font-weight: 600;">信息预览</span>
          </template>
        ...
        <div style="margin-top: 20px; display: flex; justify-content: space-between;">
          ...
          <div v-else />
          <ElButton v-if="currentStep < steps.length - 1" type="primary" @click="nextStep">
            下一步
          </ElButton>
          <ElButton v-else type="success" native-type="submit">
            确认提交
          </ElButton>
        </div>
      </form>
