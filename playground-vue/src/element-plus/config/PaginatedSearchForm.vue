<template>
  <div>
    <h2>分页搜索数据源</h2>
    <p style="color: #909399; margin-bottom: 16px; font-size: 14px;">大数据量远程搜索 / 分页加载 / 搜索防抖</p>
      ...
      <form @submit.prevent="handleSubmit" novalidate>
        <FormField v-slot="{ field }" name="userId">
          <el-form-item label="选择用户" :required="field.required">
            <el-select :model-value="(field.value as string)" filterable :filter-method="handleSearch" @change="(v: string) => field.setValue(v)" :loading="loading" :disabled="mode === 'disabled'" style="width: 400px" placeholder="输入关键词搜索（共 1000 条模拟数据）">
