...
const schema = computed<FormSchema>(() => ({
  ...
  fields: {
    username: { type: 'string', label: '用户名', required: true, component: 'Input', wrapper: 'FormItem', placeholder: '试试 admin / test', rules: [{ minLength: 3, maxLength: 20, message: '3-20 字符' }, { asyncValidator: async (v: unknown, _r: unknown, _c: unknown, signal: AbortSignal) => {
      await delay(800, signal)
      if (REGISTERED.includes(String(v).toLowerCase()))
        return `"${v}" 已被注册`
      return undefined
    }, trigger: 'blur', debounce: 300 }] },
    email: { type: 'string', label: '邮箱', required: true, component: 'Input', wrapper: 'FormItem', placeholder: '试试 admin@test.com', rules: [{ format: 'email', message: '无效邮箱' }, { asyncValidator: async (v: unknown, _r: unknown, _c: unknown, signal: AbortSignal) => {
      if (!v)
        return undefined
      await delay(800, signal)
    ...
    inviteCode: { type: 'string', label: '邀请码', required: true, component: 'Input', wrapper: 'FormItem', placeholder: 'INVITE2024 / VIP888', rules: [{ minLength: 3, message: '至少 3 字符' }, { asyncValidator: async (v: unknown, _r: unknown, _c: unknown, signal: AbortSignal) => {
      if (!v)
        return undefined
