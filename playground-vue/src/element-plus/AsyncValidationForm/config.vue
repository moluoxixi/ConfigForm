<template>
  <div>
    ...
    <p style="color: #909399; margin-bottom: 16px; font-size: 14px;">
      用户名唯一性 / 邮箱可用性 / 邀请码 / 防抖 + AbortSignal — ConfigForm + ISchema 实现
    </p>
...
/**
 * 异步验证 — Config 模式（Element Plus）
 *
 * 使用 ConfigForm + ISchema 实现异步验证：
 * - 用户名唯一性检查（防抖 300ms + AbortSignal）
 * - 邮箱可用性检查（防抖 500ms）
 * - 邀请码有效性验证（防抖 300ms）
...
const schema: ISchema = {
  ...
  decoratorProps: {
    ...
    description: '已注册用户名: admin / test / root，有效邀请码: INVITE2024 / VIP888',
  },
  properties: {
    username: {
      type: 'string',
      ...
      rules: [
        ...
        {
          asyncValidator: async (v: unknown, _r: unknown, _c: unknown, signal: AbortSignal) => {
