<template>
  <div>
    ...
    <ElCard shadow="never" style="margin-bottom: 16px">
      <ElSpace>
        <span style="font-weight: 600">变更摘要：</span><ElTag v-if="changedFields.length === 0" type="success">
          无变更
        </ElTag><template v-else>
          <ElTag type="warning">
            {{ changedFields.length }} 个已修改
          </ElTag><ElTag v-for="d in changedFields" :key="d.name" type="danger">
            {{ d.label }}
          </ElTag>
        </template>
      </ElSpace>
    ...
    <FormProvider :form="form">
      <form novalidate @submit.prevent="handleSubmit">
        <FormField v-for="d in FIELD_DEFS" :key="d.name" v-slot="{ field }" :name="d.name">
          <ElFormItem :label="d.label" :style="{ background: isChanged(d.name) ? '#fffbe6' : undefined, padding: isChanged(d.name) ? '4px 8px' : undefined, borderRadius: '4px' }" :help="isChanged(d.name) ? `原始值: ${String(ORIGINAL[d.name] ?? '—')}` : undefined">
