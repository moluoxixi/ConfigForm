...
import {
  ElInput, ElButton, ElFormItem, ElCard, ElAlert, ElTag, ElDivider,
} from 'element-plus';
...
<template>
  <div>
    <h2 style="margin-bottom: 8px;">Element Plus Field 组件 - 数据处理</h2>
    ...
    <FormProvider :form="form">
      <form @submit.prevent="handleSubmit">
        <el-card shadow="never" style="margin-bottom: 20px;">
          <template #header><span style="font-weight: 600;">值解析与转换</span></template>
          ...
          <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
            ...
            <FormField name="formattedPrice" v-slot="{ field }">
              <ElFormItem :label="field.label">
                ...
                />
                <div style="font-size: 12px; color: #909399; margin-top: 4px;">
                  格式化显示: <ElTag v-if="field.format" size="small">{{ field.format(field.value) || '—' }}</ElTag>
                </div>
              </ElFormItem>
            </FormField>
          </div>
        ...
        <el-card shadow="never" style="margin-bottom: 20px;">
          <template #header><span style="font-weight: 600;">路径映射 <ElTag size="small" type="info">firstName → user.firstName</ElTag></span></template>
        ...
        <el-card shadow="never" style="margin-bottom: 20px;">
          <template #header><span style="font-weight: 600;">空值与备注</span></template>
        ...
        <div style="display: flex; gap: 12px; margin-bottom: 20px;">
          ...
          <ElButton @click="form.reset()">重置</ElButton>
        </div>
      </form>
    </FormProvider>

    <!-- 值变化日志 -->
    <el-card v-if="changeLog.length > 0" shadow="never" style="margin-bottom: 20px;">
      <template #header><strong>值变化日志（最近 8 条）</strong></template>
    ...
    <el-card v-if="submitResult" shadow="never">
      <template #header><strong>提交数据（注意路径映射和值转换）</strong></template>
