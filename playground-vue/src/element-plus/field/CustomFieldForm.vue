    ...
    <FormProvider :form="form">
      <form @submit.prevent="handleSubmit">
        ...
        <ElCard shadow="never" style="margin-bottom: 20px;">
          ...
          <!-- 自定义组件：Slider -->
          <FormField v-slot="{ field }" name="experience">
            <ElFormItem :label="field.label">
              <div style="width: 100%; display: flex; align-items: center; gap: 16px;">
                <ElSlider
                  :model-value="(field.value as number)"
                  :min="0"
                  :max="30" :step="1" show-stops
                  :marks="{ 0: '0年', 5: '5年', 10: '10年', 20: '20年', 30: '30年' }" style="flex: 1;"
                  @update:model-value="field.setValue($event)"
                />
          ...
          <!-- 自定义组件：Rate -->
          <FormField v-slot="{ field }" name="satisfaction">
            <ElFormItem :label="field.label">
              <ElRate
                :model-value="(field.value as number)"
                show-text
