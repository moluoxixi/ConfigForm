...
import {
  ElInput, ElInputNumber, ElSelect, ElOption, ElButton, ElButtonGroup,
  ElFormItem, ElCard, ElDescriptions, ElDescriptionsItem, ElTag, ElMessage,
} from 'element-plus';
...
<template>
  <div>
    <h2 style="margin-bottom: 8px;">Element Plus Field 组件 - 模式切换</h2>
    ...
    <div style="margin-bottom: 20px; display: flex; align-items: center; gap: 12px;" role="radiogroup" aria-label="表单模式">
      ...
      <ElButtonGroup>
        <ElButton
          ...
          @click="switchPattern(p.key)"
          role="radio" :aria-checked="currentPattern === p.key"
        >
          {{ p.label }}
        </ElButton>
      </ElButtonGroup>
    ...
    <FormProvider :form="form">
      <form @submit.prevent="handleSubmit">
        ...
        >
          <template #header><span style="font-weight: 600;">{{ group.title }}</span></template>
        ...
        <ElButton v-if="currentPattern === 'editable'" type="primary" native-type="submit">提交</ElButton>
      </form>
    </FormProvider>
