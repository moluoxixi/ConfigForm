<template>
  <div>
    ...
    <div style="display: grid; grid-template-columns: 1fr 340px; gap: 24px;">
            ...
            <FormField v-slot="{ field }" name="published">
              <ElFormItem :label="field.label">
                <ElSwitch
                  ...
                  active-text="发布"
                  inactive-text="草稿" @update:model-value="field.setValue($event)"
                />
              </ElFormItem>
            </FormField>
      ...
      <!-- 事件日志 -->
      <ElCard shadow="never" style="height: fit-content;">
        <template #header>
          <div style="display: flex; justify-content: space-between; align-items: center;">
            <strong>事件日志</strong>
            <ElButton size="small" text @click="eventLog = []">
              清空
            </ElButton>
          </div>
        </template>
