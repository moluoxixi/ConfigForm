...
function drawing(e: MouseEvent): void {
  if (!isDrawing || mode.value === 'disabled')
    return
  const ctx = getCtx()
  if (!ctx)
    return
...
async function handleSubmit(): Promise<void> {
  ...
  result.value = res.errors.length > 0 ? `验证失败: ${res.errors.map(e => e.message).join(', ')}` : JSON.stringify({ ...res.values, signatureData: signatureData.value ? '[Base64]' : '' }, null, 2)
}
</script>
