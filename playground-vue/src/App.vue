...
<template>
  <div style="max-width: 1100px; margin: 0 auto; padding: 24px; font-family: system-ui, sans-serif;">
    ...
    <div style="display: flex; gap: 8px; margin-bottom: 20px; padding: 12px 16px; background: #f5f5f5; border-radius: 8px;">
      ...
      <button @click="switchUI('antd-vue')"
        :style="{
          padding: '6px 20px', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', fontWeight: 600,
      ...
      <button @click="switchUI('element-plus')"
        :style="{
          padding: '6px 20px', borderRadius: '6px', cursor: 'pointer', fontSize: '14px', fontWeight: 600,
    ...
    <div v-for="group in demoGroups" :key="group.title" style="margin-bottom: 12px;">
      <div style="font-size: 13px; font-weight: 600; color: #666; margin-bottom: 6px;">{{ group.title }}</div>
      <div style="display: flex; gap: 6px; flex-wrap: wrap;">
        <button v-for="item in group.items" :key="item.key" @click="currentDemo = item.key"
          :style="{
            padding: '6px 14px', border: '1px solid #ddd', borderRadius: '6px', cursor: 'pointer', fontSize: '13px',
    ...
    <div style="border: 1px solid #eee; border-radius: 8px; padding: 24px; background: #fff; margin-top: 12px;">
      <component :is="currentComponent" :key="`${currentUI}-${currentDemo}`" v-if="currentComponent" />
